    ...
    if isinstance(payload, dict):
        ...
        if "success" in payload or "errorcode" in payload:
            result += (
                ...
                f" errorcode={payload.get('errorcode')!r}"
            )
        return result
...
def install_vx_chunked_request(inner: Any) -> None:
    ...
    inner._request = lambda url, method="POST", data_str=None, encrypt=False, is_login=False: vx_chunked_request(
        inner, url, method, data_str, encrypt, is_login
    )
...
class Vx800vEXClientGCM:
    ...
    def _request(self, url, method="POST", data_str=None, encrypt=False, is_login=False):
        return vx_chunked_request(
            self._inner, url, method, data_str, encrypt, is_login
        )
